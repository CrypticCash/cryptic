...
#if defined(NDEBUG)
# error "Cryptic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6026942a0b206a0752f6dcdf4b6fd3c02a3be3e6dbf829200a75ee3ef8007f07");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryptic Signed Message:\n";
...
{
    // Cryptic: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Cryptic: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Cryptic: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Cryptic: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptic: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryptic: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcca635dff6ae01455a30f5ab1ac72bcbb71d07b71c86bc27206280bd71b2920b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ee2942c389408930032001b41c9fc480d24643bd3685d6396528e8817448e6e3c71db5d63daaf97a0742b5fb9f9c85a7a4d4641b3c082541504364b820120aee") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521904896;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085017239;

        if (fTestNet)
        {
            block.nTime    = 1521904853;
            block.nNonce   = 386643231;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xed8b7ccd6362611cf387535a438ef7c5016c6d9f0764cb7bd21f0ba8d03e33d0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb8, 0xd3 }; // Cryptic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrypticMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrypticMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrypticMiner(CWallet *pwallet)
{
    printf("CrypticMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptic-miner");
    ...
    try { loop {
        ...
        printf("Running CrypticMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrypticMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrypticMiner, pwallet));
}
